        ...
        <table className="w-full text-left border-separate border-spacing-0">
          <thead>
            <tr className="bg-slate-50/50 border-b border-slate-100">
              <th className="px-8 py-6 text-[10px] font-bold uppercase tracking-[0.2em] text-slate-400">Consultation</th>
              <th className="px-8 py-6 text-[10px] font-bold uppercase tracking-[0.2em] text-slate-400">Timing</th>
              <th className="px-8 py-6 text-[10px] font-bold uppercase tracking-[0.2em] text-slate-400">Status</th>
              <th className="px-8 py-6 text-[10px] font-bold uppercase tracking-[0.2em] text-slate-400 text-right">Reference</th>
            </tr>
          </thead>
          <tbody className="">
              ...
              initialAppointments.map((appt) => {
                ...
                return (
                  <React.Fragment key={appt.id}>
                            ...
                            <div className="font-heading font-medium text-slate-900 flex items-center gap-2">
                              ...
                              {hasNotes && (
                                <div className="flex items-center gap-1.5 px-2 py-0.5 bg-blue-50 text-blue-600 rounded-md border border-blue-100 animate-in fade-in zoom-in duration-300">
                                   ...
                                   <span className="text-[8px] font-bold uppercase tracking-widest whitespace-nowrap">Documented</span>
                                </div>
                              )}
                            </div>
                            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1">Video Consultation</p>
                          </div>
                        </div>
                      </td>
                      <td className="px-8 py-6 border-b border-slate-50">
                    ...
                    {/* Expandable Clinical Workstation */}
                    {isExpanded && (
                      <tr>
                             ...
                             <div className="grid grid-cols-1 lg:grid-cols-4">
                                ...
                                <div className="p-7 border-b lg:border-b-0 lg:border-r border-slate-100 bg-slate-50/20 space-y-4">
                                   <div className="flex items-center gap-2 mb-1">
                                      ...
                                      <h4 className="text-[10px] font-bold text-slate-900 uppercase tracking-widest">Session</h4>
                                   </div>
                                   <div className="p-3 bg-white rounded-xl border border-slate-100 shadow-sm">
                                      <p className="text-[9px] font-bold text-slate-400 uppercase tracking-tighter mb-1">Patient</p>
                                      <p className="text-sm font-medium text-slate-800">{appt.patient?.firstName} {appt.patient?.lastName}</p>
                                      <p className="text-[10px] text-slate-400 mt-0.5">{appt.patient?.user?.email}</p>
                                   </div>
                                   <div className="p-3 bg-white rounded-xl border border-slate-100 shadow-sm">
                                      <p className="text-[9px] font-bold text-slate-400 uppercase tracking-tighter mb-1">Duration</p>
                                   ...
                                   {appt.status !== 'COMPLETED' && appt.status !== 'CANCELLED' && (
                                      <Link href={`/dashboard/sessions/${appt.id}/call`} className="block">
                                         <button className="w-full py-3 bg-slate-900 text-white rounded-xl text-[10px] font-bold uppercase tracking-[0.2em] hover:bg-blue-600 transition-all flex items-center justify-center gap-2">
                                ...
                                <div className="lg:col-span-1 p-7 border-b lg:border-b-0 lg:border-r border-slate-100">
                                   <div className="flex items-center gap-2 mb-5">
                                      ...
                                      <h4 className="text-[10px] font-bold text-slate-900 uppercase tracking-widest">Patient Intake</h4>
                                   </div>
                                   {!appt.patient?.intakeCompleted ? (
                                      <div className="h-full flex flex-col items-center justify-center text-center py-6 opacity-40">
                                         ...
                                         <p className="text-[10px] text-slate-400 mt-1">Patient hasn't completed their intake form</p>
                                      </div>
                                   ) : (
                                      <div className="space-y-3 overflow-y-auto max-h-64 pr-1">
                                         {appt.patient?.primaryConcerns?.length > 0 && (
                                            <div>
                                               <p className="text-[9px] font-bold text-slate-400 uppercase tracking-tighter mb-2">Primary Concerns</p>
                                               <div className="flex flex-wrap gap-1">
                                                  {appt.patient.primaryConcerns.map((c: string) => (
                                                     <span key={c} className="px-2 py-0.5 bg-violet-50 text-violet-700 text-[9px] font-bold rounded-md border border-violet-100">{c}</span>
                                                  ))}
                                               </div>
                                            </div>
                                         )}
                                         {appt.patient?.reasonForSeeking && (
                                            <div className="p-3 bg-slate-50 rounded-xl border border-slate-100">
                                               <p className="text-[9px] font-bold text-slate-400 uppercase tracking-tighter mb-1">Reason for Seeking</p>
                                               <p className="text-xs text-slate-700 leading-relaxed">{appt.patient.reasonForSeeking}</p>
                                            </div>
                                         )}
                                         {appt.patient?.therapyGoals && (
                                            <div className="p-3 bg-slate-50 rounded-xl border border-slate-100">
                                               <p className="text-[9px] font-bold text-slate-400 uppercase tracking-tighter mb-1">Therapy Goals</p>
                                         ...
                                         {appt.patient?.currentMedications && (
                                            <div className="p-3 bg-amber-50 rounded-xl border border-amber-100">
                                               <p className="text-[9px] font-bold text-amber-600 uppercase tracking-tighter mb-1">Medications</p>
                                               <p className="text-xs text-slate-700 leading-relaxed">{appt.patient.currentMedications}</p>
                                            </div>
                                         )}
                                         {appt.patient?.mentalHealthHistory && (
                                            <div className="p-3 bg-slate-50 rounded-xl border border-slate-100">
                                               <p className="text-[9px] font-bold text-slate-400 uppercase tracking-tighter mb-1">MH History</p>
                                               <p className="text-xs text-slate-700 leading-relaxed">{appt.patient.mentalHealthHistory}</p>
                                            </div>
                                         )}
                                         <div className="flex items-center gap-2 pt-1">
                                            <div className={`text-[9px] font-bold px-2 py-1 rounded-md ${appt.patient?.previousTherapy ? 'bg-blue-50 text-blue-700 border border-blue-100' : 'bg-slate-100 text-slate-500'}`}>
                                ...
                                <div className="lg:col-span-2 p-7">
                                   <div className="flex items-center justify-between mb-5">
                                      <div className="flex items-center gap-2">
                                         ...
                                         <h4 className="text-[10px] font-bold text-slate-900 uppercase tracking-widest">Private Notes</h4>
                                      </div>
                                      <div className="flex items-center gap-3">
                                         <span className="text-[9px] font-bold text-slate-300 uppercase tracking-widest">Confidential</span>
                                         <button 
                                            onClick={() => saveNotes(appt.id)}
                                            disabled={state.loading}
                                            className={`px-5 py-1.5 rounded-xl text-[9px] font-bold uppercase tracking-widest transition-all ${
                                   ...
                                   />
                                   <p className="mt-3 text-[9px] font-bold text-slate-400 uppercase tracking-[0.3em] flex items-center gap-2">
